...
export function normalizeBalance(balance: string, decimals: number) : NormalizedTokenBalance {
    const normalized = new BigNumber(balance).div(BigNumber(10).pow(decimals));
    if (normalized.gt(1)) {
        return {
            value: balance,
