...
task('clone', 'clone a new wallet per given email')
  ...
  .setAction(async (taskArgs, hre : HardhatRuntimeEnvironment) => {
    ...
    assert(computedAddress == deployedAddress, `Wallet address mismatch, expected ${computedAddress} but got ${deployedAddress}`);

    if (taskArgs.print) {
      const gas = receipt.gasUsed.mul(receipt.effectiveGasPrice).div(1000000000);
      console.log(`Cloning "YawWallet" (tx: ${tx.hash})...: cloned at ${deployedAddress} with ${gas} gas`);
    }
